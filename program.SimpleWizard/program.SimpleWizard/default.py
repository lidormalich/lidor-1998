...
base='lidormalich.wordpress.com'
ADDON=xbmcaddon.Addon(id='program.SimpleWizard')
...
def CATEGORIES():
    link = OPEN_URL('https://github.com/lidormalicb/lidor-1998/blob/master/program.SimpleWizard/wizard.txt').replace('\n','').replace('\r','')
...
def wizard(name,url,description):
    ...
    dp.create("SimpleWizard","downloading  ",'', 'please wait')
    ...
    time.sleep(2)
    dp.update(0,"", "Extract ZIP files")
    print '======================================='
    print addonfolder
    print '======================================='
    extract.all(lib,addonfolder,dp)
    dialog = xbmcgui.Dialog()
    dialog.ok("Program complet", 'Unfortunately the only way to get the new changes to stick is', 'to force close kodi. Click ok to force Kodi to close,', 'DO NOT use the quit/exit options in Kodi., If the Force close does not close for some reason please Restart Device or kill task manaully')
    killxbmc()
