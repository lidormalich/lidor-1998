...
def add_item( action="" , title="" , plot="" , url="" ,thumbnail="" , extra="",mmsId="",page="",token="", isPlayable = False, folder=True ):
    ...
    if url.startswith("plugin://"):
        ...
        itemurl = '%s?action=%s&title=%s&url=%s&thumbnail=%s&plot=%s&extra=%s&mmsid=%s&page=%s&token=%s' % ( sys.argv[ 0 ] , action , urllib.quote_plus( title.encode('utf-8') ) , urllib.quote_plus(url) , urllib.quote_plus( thumbnail ) , urllib.quote_plus( plot ) , urllib.quote_plus( extra ), urllib.quote_plus( mmsId ), urllib.quote_plus(page ), urllib.quote_plus( token ))
        xbmcplugin.addDirectoryItem( handle=int(sys.argv[1]), url=itemurl, listitem=listitem, isFolder=folder)
