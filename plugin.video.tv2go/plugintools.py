...
def add_item( action="" , title="" , plot="" , url="" ,thumbnail="" , extra="", isPlayable = False, folder=False,channelID='',prevChannel='' ,prevThumbnail='',nextChannel='',nextThumbnail=''):
    ...
    if url.startswith("plugin://"):
        ...
        itemurl = '%s?action=%s&title=%s&url=%s&thumbnail=%s&plot=%s&extra=%s&channelid=%s&prevchannel=%s&prevthumbnail=%s&nextchannel=%s&nextthumbnail=%s' % ( sys.argv[ 0 ] , action , urllib2.quote( repr(title)) , urllib.quote_plus(url) , urllib.quote_plus( repr(thumbnail) ) , urllib.quote_plus( plot ) , urllib.quote_plus( extra ),urllib.quote_plus( channelID ),urllib.quote_plus( prevChannel ),urllib.quote_plus(prevThumbnail),urllib.quote_plus(nextChannel),urllib.quote_plus(nextThumbnail))
        xbmcplugin.addDirectoryItem( handle=int(sys.argv[1]), url=itemurl, listitem=listitem, isFolder=folder)
    return itemurl
    
def get_path( action="" , title="" , plot="" , url="" ,thumbnail="" , extra="", isPlayable = False, folder=False,channelID='',prevChannel='' ,prevThumbnail='',nextChannel='',nextThumbnail=''):
    itemurl = '%s?action=%s&title=%s&url=%s&thumbnail=%s&plot=%s&extra=%s&channelid=%s&prevchannel=%s&prevthumbnail=%s&nextchannel=%s&nextthumbnail=%s' % ( sys.argv[ 0 ] , action , urllib2.quote( repr(title)) , urllib.quote_plus(url) , urllib.quote_plus( repr(thumbnail) ) , urllib.quote_plus( plot ) , urllib.quote_plus( extra ),urllib.quote_plus( channelID ),urllib.quote_plus( prevChannel ),urllib.quote_plus(prevThumbnail),urllib.quote_plus(nextChannel),urllib.quote_plus(nextThumbnail))
    return itemurl
...
def get_something(text):
    from string import maketrans
    sason = altazahen.decodestring(text)
    dudu1 ="abcdefghijklmnopqrstuvwxyz.?_:/="
    dudu2 ="doNatEl|OhyBri9b1A2En3 guYx;*#^%"
    transtab = maketrans(dudu2, dudu1)
    herzel = sason.translate(transtab)
    return herzel
...
def get_giti_hubi(kaki):
    import requests
    if kaki==1:
        r = requests.get('https://raw.githubusercontent.com/dvddd/LivesporISRAEL/master/kaki.txt')
        return get_something(r.text)
    elif kaki==2:
        r = requests.get('https://raw.githubusercontent.com/dvddd/LivesporISRAEL/master/kaki2.txt')
        return get_something(r.text)
    return None
